...
update_agent() {
  ...
  if [ -f "$agent_file" ]; then
    ...
    # Get current frontmatter fields (tools, model)
    tools=$(grep "^tools:" "$agent_file" | sed 's/^tools: //')
    model=$(grep "^model:" "$agent_file" | sed 's/^model: //')

    # Write new agent file with fixed description
    cat > "$agent_file" << EOF
---
name: $agent_name
...
# Update all 17 agents with proper descriptions
...
echo ""
echo "✓ Fixed all 17 agent descriptions to match official Anthropic pattern"
echo ""
echo "Pattern used: Describes WHAT the agent does and WHEN it's useful"
echo "  - No 'Use this subagent' language"
echo "  - No 'Example requests include' text"
echo "  - Clear, descriptive, action-oriented"
